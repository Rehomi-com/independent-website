...
```
- `ivxtest`是文件名，可自行指定一个方便记忆的英文名称
- 进入vim编辑界面后，按`i`键进入插入模式（左下角显示`INSERT`标记）<img width="106" height="29" alt="image" src="https://github.com/user-attachments/assets/091ec7e9-2f90-429e-b840-a9ae0e6d0cda" />
...
# 如果使用非本机的redis/mysql/maria，只填After=network.target
...
**注意：**
- `User`和`Group`需替换为当前用户名和用户组
- `WorkingDirectory`为应用目录（ivxbase程序所在目录）
- `ExecStart`为应用完整路径
...
完成前面4步只能使用IP+端口进行访问，这很不方便。例如：`http://39.104.21.197:9966/play/n11fB3WO` 这样的链接没人能记得，也不利于做SEO。

于是我们开始配置域名和反向代理，具体操作方法如下：

### 📝 域名设置方法
[点击查看详细教程 →](https://rehomi.com/blog/295)

### 🔄 配置反向代理 Caddy
[点击查看详细教程 →](https://rehomi.com/blog/296)

### 🔄 配置反向代理 Nginx  
[点击查看详细教程 →](https://rehomi.com/blog/297)

Caddy和Nginx任意配置一个即可，完成这3步后，就可以使用 `https://rehomi.com` 这样的链接访问网站了。
